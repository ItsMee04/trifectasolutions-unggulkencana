...
export function usePembelianDariLuarToko() {
    ...
    const fetchKodeTransaksi = async () => {
        ...
        try {
            const response =
                await pembeliandariluartokoService.getKodeTransaksi();
    ...
    const fetchOptions = async (
        ...
        if (!force) {
            ...
            if (type === "pelanggan" && pelangganOptions.value.length > 0)
                return;
        }
    ...
    const fetchMasterHarga = async () => {
        ...
        try {
            ...
            masterHarga.value = Array.isArray(response)
                ? response
                : response.data;
        } catch (error) {
    ...
    const validateForm = () => {
        ...
        if (
            ...
            String(formProduk.hargabeli).trim() === ""
        ) {
            errors.value.hargabeli = "Harga beli tidak boleh kosong.";
        } else {
            const hargaRegex = /^\d+$/;
            if (!hargaRegex.test(formProduk.hargabeli)) {
                errors.value.hargabeli =
                    "Harga beli harus berupa angka tanpa simbol.";
            }
        }
    ...
    const handleSubmitProduk = async () => {
        ...
        try {
            ...
            if (isEdit.value) {
                ...
                response =
                    await pembeliandariluartokoService.updatePembelianDetail(
                        payload,
                    );
            } else {
                // Mode Tambah: Kirim Payload saja
                response =
                    await pembeliandariluartokoService.storeProdukToPembelianDetail(
                        payload,
                    );
            }
    ...
    const fetchPembelianDetail = async () => {
        ...
        try {
            const response =
                await pembeliandariluartokoService.getPembelianDetail();
            pembeliandetail.value = Array.isArray(response)
                ? response
                : response.data || [];
        } catch (error) {
    ...
    const handleEdit = async (item) => {
        ...
        const foundJenis = jenisprodukList.value.find(
            (jp) => jp.value === produk.jenisproduk_id,
        );
        formProduk.jenisproduk = foundJenis ? foundJenis : null;

        // 2. Karat
        const foundKarat = karatList.value.find(
            (k) => k.value === produk.karat_id,
        );
        ...
        // 4. Kondisi
        const foundKondisi = kondisiList.value.find(
            (kon) => kon.value === item.kondisi_id,
        );
    ...
    const handleDelete = async (item) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                await pembeliandariluartokoService.batalPembelianDetail(
                    payload,
                );
    ...
    const paymentPembelian = async () => {
        ...
        try {
            const actualId =
                formDariLuarToko.selectedId?.value ||
            ...
            const response =
                await pembeliandariluartokoService.paymentPembelian(payload);
            ...
            if (response.status) {
                ...
                const daftarProduk = pembeliandetail.value
                    .map((item, index) => {
                        ...
                        const beratItem = Number(
                            item.berat || item.produk?.berat || 0,
                        );
...
_Notifikasi Otomatis Sistem Pembelian_`;
    ...
    const handlePrintNota = async () => {
        ...
        try {
            ...
            const response =
                await pembeliandariluartokoService.CetakNotaPembelian(payload);
            if (response.url) {
