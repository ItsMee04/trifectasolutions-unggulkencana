    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                <div class="card-title mb-0"><b>DAFTAR JENIS PRODUK</b></div>
                ...
                <div class="d-flex align-items-center">
                    <div class="input-group input-group-sm" style="width: 250px;">
                        ...
                        <input type="text" class="form-control border-start-0 ps-0" placeholder="Cari Jenis Produk..."
                            v-model="searchQuery" />
                    </div>
                </div>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table text-nowrap table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <tr v-else-if="paginatedJenisProduk.length === 0">
                                <td colspan="4" class="text-center">Tidak ada data.</td>
                            </tr>
                            <tr v-else v-for="(item, index) in paginatedJenisProduk" :key="item.id">
                                <td scope="row">{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td>
                                <td>
                                    <div class="productimgname">
                                        <a href="javascript:void(0);" class="product-img stock-img">
                                            <img :src="item.image ? `/storage/images/jenisproduk/${item.image}?t=${new Date().getTime()}` : BASE_DEFAULT_IMAGE_URL"
                                                :alt="item.jenis">
                                        </a>
                ...
                <div v-if="filteredJenisProduk.length > 0" class="d-flex justify-content-between align-items-center p-3">
                    <div class="text-muted small">
                        Showing {{ (currentPage - 1) * itemsPerPage + 1 }} to
                        {{ Math.min(currentPage * itemsPerPage, filteredJenisProduk.length) }} of
                        {{ filteredJenisProduk.length }} entries
                    </div>
