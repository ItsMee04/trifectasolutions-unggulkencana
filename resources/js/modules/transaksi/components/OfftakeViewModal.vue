<template>
    <teleport to='body'>
        <div class="modal fade" id="offtakeviewModal" tabindex="-1" aria-labelledby="offtakeviewModalLabel"
            aria-hidden="true">
                                        ...
                                        <div>
                                            <div class="row sales-details-items d-flex">
                                                <div class="col-md-4 details-item">
                                                    <h6>SUPLIER</h6>
                                                    <h4 class="mb-1">{{ selectedTransaction?.suplier?.nama }}</h4>
                                                    <p class="mb-0">{{ selectedTransaction?.suplier?.alamat }}</p>
                                                    <p class="mb-0">{{ selectedTransaction?.suplier?.kontak }}</p>
                                                </div>
                                            ...
                                            <div class="table-responsive no-pagination mb-3">
                                                <table class="table" id="transaksiProduk">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <template
                                                            v-else-if="selectedTransaction && selectedTransaction.offtakedetail && selectedTransaction.offtakedetail.length > 0">
                                                            <tr v-for="(item, index) in selectedTransaction.offtakedetail"
                                                                :key="index">
                                                                <td>{{ item.produk?.kodeproduk || '-' }}</td>
                                                                <td>{{ item.produk?.nama || '-' }}</td>
                                                                <td>{{ item.produk?.berat || '-' }}</td>
                                                                <td>{{ item.produk?.karat?.karat || '-' }}</td>
                                                        ...
                                                        <ul class="border-1 rounded-1">
                                                            <li class="border-bottom d-flex justify-content-between">
                                                                ...
                                                                <h5 class="text-danger">
                                                                    {{ formatRupiah(selectedTransaction?.hargatotal) }}
                                                                </h5>
                                                            </li>
                                                            <li class="border-bottom d-flex justify-content-between">
                                                                <h4 class="border-end">Keterangan</h4>
