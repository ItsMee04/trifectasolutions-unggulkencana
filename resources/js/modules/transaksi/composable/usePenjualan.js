...
export function usePenjualan() {
    ...
    const fetchPenjualan = async () => {
        ...
        try {
            ...
            penjualan.value = Array.isArray(response) ? response : (response.data || []);
        } catch (error) {
            penjualan.value = [];
        } finally {
    ...
    const handleView = (item) => {
        ...
        const modalElement = document.getElementById('penjualanviewModal');
        if (modalElement) {
    ...
    const totalPages = computed(() => {
        ...
        const filteredCount = penjualan.value.filter(item =>
    ...
    return {
        ...
        selectedTransaction,
        filteredPenjualan: computed(() => {
            const query = searchQuery.value.toLowerCase();
            return penjualan.value.filter(item =>
                String(item.kode || '').toLowerCase().includes(query)
            );
        }),
        paginatedPenjualan: computed(() => {
