...
export function usePembelian() {
    ...
    const fetchPembelian = async () => {
        ...
        try {
            ...
            pembelian.value = Array.isArray(response) ? response : (response.data || []);
        } catch (error) {
            pembelian.value = [];
        } finally {
    ...
    const handleView = (item) => {
        ...
        const modalElement = document.getElementById('pembelianviewModal');
        if (modalElement) {
    ...
    const handlePrintNota = async () => {
        ...
        try {
            const { url } = await pembelianService.CetakNotaPembelian(payload)
    ...
    const handleBatalTransaksi = async (item) => {
        ...
        if (result.isConfirmed) {
            ...
            try {
                ...
                await pembelianService.batalTransaksi(payload);
    ...
    const totalPages = computed(() => {
        ...
        const filteredCount = pembelian.value.filter(item =>
    ...
    return {
        ...
        selectedTransaction,
        filteredPembelian: computed(() => {
            const query = searchQuery.value.toLowerCase();
            return pembelian.value.filter(item =>
                String(item.kode || '').toLowerCase().includes(query)
            );
        }),
        paginatedPembelian: computed(() => {
