<template>
    <div class="content container-fluid">
        <div class="page-header">
            <div class="add-item d-flex">
                <div class="page-title">
                    <h4>Halaman Offtake</h4>
                    <h6>Kelola Transaksi Anda</h6>
                </div>
            </div>
            <ul class="table-top-head">
                <li>
                    <a data-bs-toggle="tooltip" data-bs-placement="top" title="Refresh" @click="handleRefresh"><i
                            data-feather="rotate-ccw" class="feather-rotate-ccw"></i></a>
                </li>
            </ul>
            <div class="page-btn import">
                <a href="#" class="btn btn-added color" @click="handleCreate">
                    <i data-feather="plus-circle" class="me-2"></i>Tambah Transaksi
                </a>
            </div>
        </div>

        <div class="row">
            <OfftakeForm />
        </div>
    </div>

    <OfftakeModal />
    <OfftakePaymentCompleteModal />
</template>

<script setup>
import { onMounted } from 'vue';
import { useOfftake } from '../composables/useOfftake';
import OfftakeForm from '../components/OfftakeForm.vue';
import OfftakeModal from '../components/OfftakeModal.vue';
import OfftakePaymentCompleteModal from '../components/OfftakePaymentCompleteModal.vue'

const { handleCreate } = useOfftake();
</script>
